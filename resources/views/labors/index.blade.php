    ...
    @csrf
      <div class="row" >
        <div class="form-group">
          ...
          <div class="col-sm-3">
            <input type="search" name="phone_number" id="search_phone" placeholder="Search By Phone" class="form-control">
          </div>
...
<h3 class="box-title" for="table_all">List of All Labor</h3>
    <div id="table_all" class="dataTables_wrapper form-inline dt-bootstrap" style="background-color:rgb(247, 248, 249); padding: 10px;">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($labors as $labor)
                <tr role="row" class="odd">
                 ...
                 <form class="row" method="POST" action="{{ route('labors.destroy', ['id' => $labor->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('labors.edit', ['id' => $labor->id]) }}" class="btn btn-warning col-xs" style="margin-left: 5px;">
                        Edit
                        </a>
                         <button type="submit" class="btn btn-danger col-xs" style="margin-left: 5px; margin-top: 5px;">
                          Delete
                        </button>
                        <a href="{{ route('labors.edit', ['id' => $labor->id]) }}" class="btn btn-primary col-xs" style="margin-left: 5px; margin-top: 5px;">
                        View
                        </a>
                    </form>
                  </td>
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($labors)}} of {{count($labors)}} entries</div>
        </div>
        <div class="col-sm-7">
