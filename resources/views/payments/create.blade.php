...
@section('content')
    ...
    <div class="box box-primary">
        <br/>
        <h3 align="center">Add Customer Payment</h3>
        <br/>
    ...
    <script>
        $(document).ready(function () {
            ...
            function dynamic_field(number) {
                html = '<tr>';
                 html += '<td><select name="project_id[]" id="project_id" class="form-control" required><option value="">Select Project</option>@foreach($projects as $project)<option value="{{$project->id}}">{{ $project->title }}</option>@endforeach </select></td>';
                html += '<td><input type="number" name="received_amount[]" class="form-control"/></td>';
