    ...
    @csrf
      <div class="row" >
        <div class="form-group">
          <div class="col-sm-3">
            <input type="search" name="search_name" id="search_name" placeholder="Search By Name" class="form-control">
          </div>

          <div class="col-sm-3">
            <input type="search" name="search_materil" id="search_material" placeholder="Search By Material" class="form-control">
          </div>
...
<h3 class="box-title" for="table_all">List of All Suppliers</h3>
    <div id="table_all" class="dataTables_wrapper form-inline dt-bootstrap" style="background-color:rgb(247, 248, 249); padding: 10px;">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">            
               <th width="10%" class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Name: activate to sort column descending" aria-sort="ascending">Name</th>
                ...
                <th width="8%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Material: activate to sort column ascending">Material</th> 
            ...
            @foreach ($suppliers as $supplier)
                <tr role="row" class="odd">
                 ...
                 <form class="row" method="POST" action="{{ route('suppliers.destroy', ['id' => $supplier->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('suppliers.edit', ['id' => $supplier->id]) }}" class="btn btn-warning col-xs" style="margin-left: 5px;">
                        Edit
                        </a>
                         <button type="submit" class="btn btn-danger col-xs" style="margin-left: 5px; margin-top: 5px;">
                          Delete
                        </button>
                        <a href="{{ route('suppliers.edit', ['id' => $supplier->id]) }}" class="btn btn-primary col-xs" style="margin-left: 5px; margin-top: 5px;">
                        View
                        </a>
                    </form>
                  </td>
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($suppliers)}} of {{count($suppliers)}} entries</div>
        </div>
        <div class="col-sm-7">
